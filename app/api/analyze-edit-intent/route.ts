...
export async function POST(request: NextRequest) {
  try {
    const { prompt, manifest, model, config }: { prompt: string; manifest: FileManifest; model: LLMModel; config: LLMModelConfig } = await request.json();
    
    console.log('[analyze-edit-intent] Request received');
    console.log('[analyze-edit-intent] Prompt:', prompt);
    console.log('[analyze-edit-intent] Model:', model.id);
    console.log('[analyze-edit-intent] Manifest files count:', manifest?.files ? Object.keys(manifest.files).length : 0);
    
    if (!prompt || !manifest || !model || !config) {
      return NextResponse.json({
        error: 'prompt, manifest, model, and config are required'
      }, { status: 400 });
    }
    ...
    console.log('[analyze-edit-intent] Using AI model:', model.id);
