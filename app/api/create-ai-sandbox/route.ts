...
async function createSandboxInternal() {
  ...
  try {
    ...
    // Create E2B sandbox
    console.log(`[create-ai-sandbox] Creating E2B sandbox with ${appConfig.E2BProvider.timeoutMinutes} minute timeout...`);
    
    // Prepare sandbox configuration
    const sandboxConfig: any = {
      timeout: appConfig.E2BProvider.timeoutMs,
      runtime: appConfig.E2BProvider.runtime,
      ports: [appConfig.E2BProvider.devPort]
    };
    ...
    // Get the sandbox URL using the correct E2B Sandbox API
    const sandboxUrl = sandbox.domain(appConfig.E2BProvider.devPort);
...
// E2B Sandbox compatible Vite configuration
export default defineConfig({
  plugins: [react()],
  server: {
    host: '0.0.0.0',
    port: ${appConfig.E2BProvider.devPort},
    strictPort: true,
    hmr: true,
    allowedHosts: [
      'localhost',
      '127.0.0.1',
      '` + sandboxHostname + `', // Allow the E2B Sandbox domain
...
body {
  ...
  background-color: rgb(17 24 39);
    ...
    await new Promise(resolve => setTimeout(resolve, appConfig.E2BProvider.devServerStartupDelay));
